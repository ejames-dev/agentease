...
- Optional `sentry` extra (`pip install agentease[sentry]`) that forwards
  provider and workflow-pipeline exceptions to a Sentry client the host
  application has already initialized. AgentEase never initializes its own
  client or ships a DSN; with the extra absent this is a no-op.
